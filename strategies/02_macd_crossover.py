...
class MACDStrategy:
    def __init__(self, symbol='EURUSD=X', fast=12, slow=26, signal=9):
        self.symbol = symbol
        self.fast = fast
        self.slow = slow
        self.signal = signal
    ...
    def on_bar(self, bar, macd, signal_line):
    ...
    def should_enter(self, bar, macd, signal_line):
        """Check entry conditions"""
        if self.prev_macd is None:
            return False
        return self.position is None and self.prev_macd <= self.prev_signal and macd > signal_line
    
    def should_exit(self, bar, macd, signal_line):
        """Check exit conditions"""
        if self.prev_macd is None:
            return False
        return self.position == 'LONG' and self.prev_macd >= self.prev_signal and macd < signal_line

def run_backtest(symbol='EURUSD=X', start_date='2025-02-03', end_date='2026-02-03'):
    """Run backtest with real market data"""
    print(f"\n{'='*60}")
    print(f"MACD CROSSOVER STRATEGY BACKTEST")
    print(f"{'='*60}")
    ...
    # Initialize strategy
    strategy = MACDStrategy(symbol=symbol)
    
    # Calculate MACD
    data['MACD'], data['Signal'], data['Histogram'] = strategy.calculate_macd(data)
    ...
    for i in range(len(data)):
        ...
        signal = strategy.on_bar(bar, macd, signal_line)
