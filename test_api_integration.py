    ...
    if response.status_code == 200:
        ...
        if access_token:
            ...
            try:
                ...
                print(f"✓ Using DRF Token authentication")
            except:
...
try:
    ...
    if response.status_code == 200:
        print(f"✓ User info endpoint working (Status: {response.status_code})")
        data = response.json()
...
try:
    ...
    if response.status_code == 200:
        print(f"✓ Strategies list endpoint working (Status: {response.status_code})")
        data = response.json()
        ...
        print(f"✗ Failed to get strategies (Status: {response.status_code})")
        print(f"  Response: {response.content}")
except Exception as e:
    print(f"✗ Error: {e}")
...
try:
    ...
    if response.status_code == 200:
        print(f"✓ Backtest health endpoint working (Status: {response.status_code})")
        data = response.json()
        print(f"  Status: {data.get('status')}")
    else:
        print(f"⚠ Backtest health (Status: {response.status_code})")
except Exception as e:
    print(f"⚠ Backtest check: {e}")
...
'''
    ...
    if response.status_code in [200, 201]:
        print(f"✓ Strategy creation endpoint working (Status: {response.status_code})")
        data = response.json()
...
print("✓ Strategy endpoints: Accessible")
print("\nKey Findings:")
